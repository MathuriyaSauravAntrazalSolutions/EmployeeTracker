...
export default class Lookup extends NavigationMixin(LightningElement) {
    ...
    @api
    set emailSelection(initialSelection) {
        if (initialSelection) {
            this._curEmailSelection = Array.isArray(initialSelection) ? initialSelection : [initialSelection];
            this.processEmailSelectionUpdate(false);
        }
    }

    get emailSelection() {
        return this._curEmailSelection;
    }
    ...
    processEmailSelectionUpdate(isUserInteraction) {
        // Reset search
        this._cleanSearchTerm = '';
        this._searchTerm = '';
        this.setSearchResults([...this._defaultSearchResults]);
        ...
        // If selection was changed by user, notify parent components
        if (isUserInteraction) {
            const selecteEmaildIds = this._curEmailSelection.map((sel) => sel.title);
            console.log(selecteEmaildIds);
            this.dispatchEvent(new CustomEvent('emailselectionchange', { detail: selecteEmaildIds }));
        }
    }
    ...
    handleKeyDown(event) {
        ...
        if (event.keyCode === KEY_ARROW_DOWN) {
            ...
            const selectedItem = this._curEmailSelection.find((result) => result.title === event.target.value); // -------------------------------------------------
            if (selectedItem) {
                return;
            }
            this.keyIndex+=1;
            const sbtle = 'Email • '+event.target.value;
            const selectedEmailItem = {icon: 'utility:email', id: this.keyIndex, sObjectType: '', subtitle: sbtle, title: event.target.value, titleFormated: event.target.value};   // -------------------------------------------------
            const newSelection = [...this._curEmailSelection];
            console.log(newSelection);
            newSelection.push(selectedEmailItem);
            this._curEmailSelection = newSelection;
            console.log(this._curEmailSelection);
            this.processEmailSelectionUpdate(true);
        }
    }
    ...
    handleRemoveSelectedEmailItem(event) {
        if (this.disabled) {
            return;
        }
        const recordId = event.currentTarget.name;
        this._curEmailSelection = this._curEmailSelection.filter((item) => item.id !== recordId);
        // Process selection update
        this.processEmailSelectionUpdate(true);
    }
